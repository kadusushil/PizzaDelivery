 ...
 **/
 user._users.get = (data, callback) => {
   ...
   var email = typeof(queryString.email) == 'string' && queryString.email.trim().length > 0
 ...
 **/
 user._users.put = (data, callback) => {
   ...
   var tokenId = typeof(data.headers.token) == 'string' && data.headers.token.trim().length == 20
                 ? data.headers.token : false;

   if (tokenId) {
     // verify token expiry
     if (authService.verifyToken(tokenId, (isTokenValid) => {
       ...
       if (isTokenValid) {
         ...
         if (!helpers.isSatisfyPasswordPolicy(password)) {
          ...
          _data.readFile('tokens', tokenId, (error, tokenData) => {
            if (!error) {
              ...
              callback(400, {'error':'not able to retrieve data'});
            }
          });
       } else {
    ...
    callback(400, {'error':'Token not valid or token is missing'});
   }
 };
